...
const raw = (value: bigint | number): Fxp =>
  fxpFromRaw(BigInt(value) * 10_000n);
...
describe("trait calculation fractions", () => {
  it.each([
    ...
    "normalizes pinned signs and zero denominators without gcd reduction",
    (input, expected) => {
  ...
  it("adds fractions with pinned fixed-point operations and does not reduce", () => {
    expect(addFractions(fraction(1, 2), fraction(1, 3))).toEqual(
      fraction(5, 6),
    );
  });

  it("multiplies fractions with pinned fixed-point operations and does not reduce", () => {
    expect(multiplyFractions(fraction(2, 3), fraction(3, 4))).toEqual(
      fraction(6, 12),
    );
  });

  it("divides fractions by swapping the normalized right operand", () => {
    expect(divideFractions(fraction(2, 3), fraction(4, 5))).toEqual(
      fraction(10, 12),
    );
    expect(divideFractions(fraction(2, 3), fraction(0, 5))).toEqual(
      fraction(10, 0),
    );
  });
  ...
  it("preserves signed-64 wrap for additions inside fraction addition", () => {
    ...
    expect(rawValue(addFractions(left, right).numerator)).toBe(
      FXP_MIN_RAW + 9_999n,
    );
  });
  ...
  it.each([
    ...
    "preserves signed-64 saturation for fraction multiplication",
    (boundary, factor, expected) => {
  ...
  it("normalizes the minimum raw denominator through pinned saturated multiplication", () => {
    expect(
      normalizeFraction({
        numerator: raw(1),
        denominator: fxpFromRaw(FXP_MIN_RAW),
      }),
    ).toEqual({
      numerator: raw(-1),
